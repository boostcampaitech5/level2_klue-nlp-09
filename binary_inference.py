...
from utils import seed_everything, load_yaml, load_yaml_type, concat, concat_3, post_submission
...
def main(config):
    ...
    ## load my model
    model_dir = f"./model/{config.inference_model_name}"
    ...
    output_dir = f"./prediction/binary_submission.csv"
    output.to_csv(output_dir, index=False)  # 최종적으로 완성된 예측한 라벨 csv 파일 형태로 저장.
    #### 필수!! ##############################################
    print("---- Finish! ----")

def binary_main(config):
    """
    주어진 dataset csv 파일과 같은 형태일 경우 inference 가능한 코드입니다.
    """
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
